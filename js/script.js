...
const playAgainButton = document.getElementById('play-again-button'); // New reference
...
function init() {
    ...
    playAgainButton.addEventListener('click', () => {
        sessionStorage.setItem('skipInitialPopup', 'true'); // Set flag to skip initial popup
        location.reload(); // Refresh the page
    });

    // Check if we should skip the initial popup
    const skipInitialPopup = sessionStorage.getItem('skipInitialPopup');
    if (skipInitialPopup === 'true') {
        sessionStorage.removeItem('skipInitialPopup'); // Remove the flag
        startGame(); // Start the game immediately
    } else {
        initialPopup.style.display = 'block'; // Show the initial popup
    }
}
...
function renderTiles() {
    ...
    tiles.forEach((tileNumber, index) => {
        ...
        if (index === emptyTileIndex) {
            ...
            tileElement.style.backgroundSize = `${GRID_SIZE * tileSize}px ${GRID_SIZE * tileSize}px`;
